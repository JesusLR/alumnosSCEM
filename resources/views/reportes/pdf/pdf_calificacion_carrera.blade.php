  ...
  <body>
    ...
    <div class="row">
      ...
      <p align="justify">{{$suscribe}} {{$firmante->firNombre}}, {{$firmante->firPuesto}}
         ...
         {{$depClaveOficial}}; hace constar que {{$joven}} joven {{$nombreCompleto}} {{$alumnoEsFue}}
        {{$alumno}} del {{$materiaCurso}} curso de la {{$progTituloOficial}}
        en el ciclo escolar del {{$cicloEscolar}} y ha obtenido las siguientes calificaciones:</p>
          <br>
      <p><b>ASIGNATURAS</b> <span style="float:right;">CALIFICACIONES</span></p>
      {{-- @foreach($materiasChunks as $materias) --}}
      @foreach ($semestre as $materias)
        @php
          $firstMateria = $materias->first();
          $cicloSemestre = $firstMateria["cicloSemestre"];
        @endphp
    ...
    <br>
    <p>Calificación mínima para aprobar una asignatura: {{$depCalMinAprob}}</p>
      <br>
